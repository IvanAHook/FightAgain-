...
public class DamageComponent : MonoBehaviour {
	...
	void OnTriggerEnter2D( Collider2D other )
	{
		...
		{
			if (other.gameObject.GetComponentInParent<Rigidbody2D>().gameObject.GetComponent<Transform>() != null &&
			other.gameObject.GetComponentInParent<Rigidbody2D>().gameObject.GetComponent<Transform>() == myOwner) return; // RIP Unity, leave as is / Arre Barre
		}
